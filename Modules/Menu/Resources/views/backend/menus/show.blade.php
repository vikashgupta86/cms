            ...
            <div class="row">
                ...
                {{-- <div class="col-4">
                    <div class="btn-toolbar float-end" role="toolbar" aria-label="Toolbar with button groups">
                        ...
                        <x-backend.buttons.list route="{{ route(" backend.$module_name.index") }}"
                            title="{{ __('menu::text.menu_list') }}" icon="fas fa-list" small="true" />
                        <x-backend.buttons.create
                            route="{{ route('backend.menuitems.create', ['menu_id' => ${$module_name_singular}->id]) }}"
                            title="{{ __('menu::text.add_menu_item') }}" icon="fas fa-plus-circle" small="true" />
                        <x-backend.buttons.edit route='{!!route("backend.$module_name.edit", ${$module_name_singular})!!}'
                            title="{{__('Edit')}} {{ $module_title }}" small="true" />
                    </div>
                </div> --}}
            </div>
            ...
            <div class="row">
                <div class="col-12 col-sm-6">
                    <div class="form-group mb-3">
                        ...
                        <p class="form-control-plaintext">{{ ${$module_name_singular}->locale ??
                            __('menu::text.all_locales') }}</p>
                    </div>
                </div>
                <div class="col-12 col-sm-6">
                    <div class="form-group mb-3">
            ...
            <div class="row mt-4">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <x-backend.buttons.create
                            route="{{ route('backend.menuitems.create', ['menu_id' => ${$module_name_singular}->id]) }}"
                            title="{{ __('menu::text.add_item') }}" icon="fas fa-plus-circle" small="true" />
                    </div>
                    ...
                    @if(${$module_name_singular}->items->count() > 0)
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                <input type="text" id="menuSearch" class="form-control" placeholder="Search menu item...">
                            </div>
                                ...
                                document.addEventListener('DOMContentLoaded', function () {
                                    ...
                                    function filterMenuItems() {
                                        ...
                                        rows.forEach(function (row) {
                                            ...
                                            if (typeValue && !rowText.includes(typeValue)) {
                                                show = false;
                                            }

                                            if (statusValue && !rowText.includes(statusValue)) {
                                                show = false;
                                            }
